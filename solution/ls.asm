...
static Header* morecore(uint nu, int use_huge_pages) {
 ...
 9d9:	74 4d                	je     a28 <morecore+0x58>
 ...
 9e8:	52                   	push   %edx
 9e9:	89 cb                	mov    %ecx,%ebx
 9eb:	e8 63 fc ff ff       	call   653 <setusehugepages>
  p = sbrk(nu * sizeof(Header));
 9f0:	8d 04 dd 00 00 00 00 	lea    0x0(,%ebx,8),%eax
 9f7:	89 04 24             	mov    %eax,(%esp)
 9fa:	e8 1c fc ff ff       	call   61b <sbrk>
  if(p == (char*)-1)
 9ff:	83 c4 10             	add    $0x10,%esp
 a02:	83 f8 ff             	cmp    $0xffffffff,%eax
 a05:	74 71                	je     a78 <morecore+0xa8>
  hp->s.size = nu;
...
void vfree(void *ap, int use_huge_pages) {
 ...
 a0d:	83 c0 08             	add    $0x8,%eax
    hugefree(ap);
 a10:	50                   	push   %eax
 a11:	e8 2a ff ff ff       	call   940 <hugefree>
    return huge_freep;
 a16:	a1 40 12 00 00       	mov    0x1240,%eax
}
 ...
 a27:	90                   	nop
    if(nu < 4096)
 a28:	ba 00 10 00 00       	mov    $0x1000,%edx
 a2d:	39 d0                	cmp    %edx,%eax
 a2f:	89 d3                	mov    %edx,%ebx
 a31:	0f 43 d8             	cmovae %eax,%ebx
  setusehugepages(use_huge_pages);
 a34:	83 ec 0c             	sub    $0xc,%esp
 a37:	6a 00                	push   $0x0
 a39:	e8 15 fc ff ff       	call   653 <setusehugepages>
  p = sbrk(nu * sizeof(Header));
 a3e:	8d 04 dd 00 00 00 00 	lea    0x0(,%ebx,8),%eax
 a45:	89 04 24             	mov    %eax,(%esp)
 a48:	e8 ce fb ff ff       	call   61b <sbrk>
  if(p == (char*)-1)
 a4d:	83 c4 10             	add    $0x10,%esp
 a50:	83 f8 ff             	cmp    $0xffffffff,%eax
 a53:	74 23                	je     a78 <morecore+0xa8>
  hp->s.size = nu;
 ...
 a5b:	83 c0 08             	add    $0x8,%eax
    normalfree(ap);
 a5e:	50                   	push   %eax
 a5f:	e8 4c fe ff ff       	call   8b0 <normalfree>
    return freep;
 a64:	a1 4c 12 00 00       	mov    0x124c,%eax
}
